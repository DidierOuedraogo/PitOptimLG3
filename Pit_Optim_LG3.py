...
st.markdown("<div class='subtitle'>Optimisation de fosses minières par l'algorithme de Lerchs-Grossmann</div>", unsafe_allow_html=True)
...
# Algorithme de Lerchs-Grossmann
def run_lerchs_grossmann(block_model, size_x, size_y, size_z, slope_angle, iterations=3):
    # Créer une copie pour ne pas modifier le modèle original
    model_copy = block_model.copy()
    optimal_pit = []
    
    # Créer une matrice 3D pour stocker les valeurs cumulatives des blocs
    cumulative_values = np.zeros((size_x, size_y, size_z))
    
    # Remplir la matrice avec les valeurs économiques initiales
    # Créer un dictionnaire pour un accès rapide
    blocks_dict = {}
    for block in model_copy:
        x, y, z = block['x'], block['y'], block['z']
        blocks_dict[(x, y, z)] = block
        cumulative_values[x, y, z] = block['value']
    
    # Calculer le dip_radius (facteur de pente) à partir de l'angle de pente
    dip_factor = 1.0 / np.tan(np.radians(slope_angle))
    
    # Itérations de l'algorithme
    for iteration in range(iterations):
        # Phase 1: Accumulation ascendante des valeurs
        for z in range(size_z-2, -1, -1):  # De bas en haut
            for y in range(size_y):
                for x in range(size_x):
                    # Pour chaque bloc, vérifier si la valeur cumulée sous lui est positive
                    block_above_value = 0
                    if z+1 < size_z:
                        # Distance horizontale maximale selon le facteur de pente
                        max_dist = int(np.ceil(dip_factor))
                        
                        # Vérifier les blocs du cône sous le bloc courant
                        for dy in range(-max_dist, max_dist+1):
                            for dx in range(-max_dist, max_dist+1):
                                ...
                                if 0 <= nx < size_x and 0 <= ny < size_y:
                                    # Vérifier si le bloc est dans le cône (distance moins que max_dist)
                                    dist = np.sqrt(dx**2 + dy**2)
                                    if dist <= max_dist:
                                        block_above_value = max(block_above_value, cumulative_values[nx, ny, nz])
                    
                    # Mettre à jour la valeur cumulative
                    if (x, y, z) in blocks_dict:
                        direct_value = blocks_dict[(x, y, z)]['value']
                        cumulative_values[x, y, z] = direct_value + max(0, block_above_value)
        
        # Phase 2: Sélection des blocs pour la fosse optimale (de haut en bas)
        for z in range(size_z):
            for y in range(size_y):
                for x in range(size_x):
                    if (x, y, z) in blocks_dict and cumulative_values[x, y, z] > 0:
                        # Ce bloc est dans la fosse optimale
                        block = blocks_dict[(x, y, z)]
                        block['in_pit'] = True
                        
                        # Si ce bloc n'est pas déjà dans la liste optimale, l'ajouter
                        if block not in optimal_pit:
                            optimal_pit.append(block)
    
    # Affiner les blocs de la fosse selon les contraintes de pente
    # Ajouter les blocs qui sont nécessaires pour maintenir la pente
    for iteration in range(1):  # Une passe supplémentaire
        temp_pit = optimal_pit.copy()
        for block in temp_pit:
            x, y, z = block['x'], block['y'], block['z']
            
            # Si le bloc est profond, vérifier les blocs au-dessus pour la pente
            if z > 0:
                # Distance horizontale maximale selon le facteur de pente
                max_dist = int(np.ceil(dip_factor))
                
                # Vérifier les blocs du cône au-dessus du bloc courant
                for dy in range(-max_dist, max_dist+1):
                    for dx in range(-max_dist, max_dist+1):
                        nx, ny, nz = x+dx, y+dy, z-1
                        ...
                        if 0 <= nx < size_x and 0 <= ny < size_y:
                            ...
                            if dist <= max_dist:
                                if (nx, ny, nz) in blocks_dict:
                                    support_block = blocks_dict[(nx, ny, nz)]
                                    if not support_block['in_pit']:
                                        support_block['in_pit'] = True
                                        optimal_pit.append(support_block)
...
with col1:
    # Explication de l'algorithme Lerchs-Grossmann avec un affichage amélioré
    with st.expander("À propos de l'algorithme Lerchs-Grossmann", expanded=False):
        st.markdown("### Algorithme de Lerchs-Grossmann")
        st.markdown("L'algorithme de Lerchs-Grossmann est une méthode classique d'optimisation de fosse minière qui utilise la théorie des graphes pour déterminer les limites maximisant la valeur économique.")
        ...
        with col1:
            ...
            st.markdown("Le modèle de blocs est transformé en un graphe où chaque bloc est un nœud avec des arcs représentant les contraintes d'extraction.")
        
        with col2:
            st.markdown("🔍 **Optimisation mathématique**")
            st.markdown("L'algorithme cherche itérativement la coupe de graphe maximisant la somme des valeurs de blocs.")
        
        with col3:
            st.markdown("🏔️ **Respect des contraintes**")
            st.markdown("Les contraintes géotechniques de pente sont intégrées dans la structure du graphe.")
        ...
        with adv_cols[0]:
            st.markdown("🎯 **Exactitude**")
            st.markdown("Produit une solution mathématiquement exacte pour le modèle de blocs donné.")
        
        with adv_cols[1]:
            st.markdown("📊 **Fiabilité**")
            st.markdown("Méthode éprouvée et robuste, utilisée dans l'industrie depuis des décennies.")
        
        with adv_cols[2]:
            st.markdown("🔄 **Polyvalence**")
            st.markdown("Applicable à une grande variété de gisements et de contraintes géotechniques.")
        ...
        with params_cols[0]:
            st.markdown("**Nombre d'itérations**")
            st.markdown("**Angle de pente**")
            st.markdown("**Contraintes additionnelles**")
        
        with params_cols[1]:
            st.markdown("Contrôle la précision et la convergence de l'algorithme. Plus d'itérations donnent généralement des résultats plus précis.")
            st.markdown("Détermine la stabilité des parois de la fosse et affecte directement la quantité de stérile à extraire.")
            st.markdown("Peut inclure des zones d'exclusion, des infrastructures existantes ou des limites de propriété.")
        
        st.info("L'algorithme a été développé par Helmut Lerchs et Ingo F. Grossmann en 1965 et reste une référence dans l'industrie minière pour l'optimisation de fosses à ciel ouvert.")

    # Paramètres d'algorithme
    st.header("Algorithme de Lerchs-Grossmann")
    
    with st.expander("Paramètres de l'algorithme", expanded=True):
        lg_iterations = st.slider("Nombre d'itérations", min_value=1, max_value=10, value=3,
                                help="Plus d'itérations donnent des résultats plus précis mais augmentent le temps de calcul.")
...
with col2:
    ...
    with results_container:
        if st.session_state.results_ready:
            ...
            st.write(f"Algorithme: **Lerchs-Grossmann** | Temps d'exécution: {st.session_state.execution_time:.2f} secondes")
            ...
            with tab1:
                ...
                if 'export_csv' in st.session_state and st.session_state.export_csv:
                    ...
                    if st.session_state.optimal_pit:
                        ...
                        csv_filename = f"{project_name_safe}_results_LG_{datetime.now().strftime('%Y%m%d')}.csv"
...
if run_optimizer:
    ...
    status_text.text("Initialisation du graphe économique...")
    progress_bar.progress(30)
    time.sleep(0.3)  # Simulation de temps de calcul
    
    status_text.text("Calcul des valeurs cumulatives...")
    progress_bar.progress(50)
    time.sleep(0.3)  # Simulation de temps de calcul
    
    status_text.text("Optimisation itérative de la fosse...")
    progress_bar.progress(70)
    
    # Exécuter l'algorithme de Lerchs-Grossmann
    st.session_state.optimal_pit, st.session_state.pit_boundary = run_lerchs_grossmann(
        st.session_state.block_model,
...
if st.session_state.model_imported:
    ...
    if view_mode == "Teneurs" or (view_mode != "Fosse optimale" and view_mode != "Limites de la fosse" and not st.session_state.results_ready):
        ...
        for i, block in enumerate(sampled_blocks):
            ...
            # Couleurs différentes pour l'algorithme Lerchs-Grossmann (teintes de vert/jaune)
            color = f'rgb({50+int(z_normalized*205)}, {100+int(z_normalized*100)}, {50})'
        ...
        for z, blocks in z_levels.items():
            ...
            # Normalisation pour coloration (couleurs vertes/jaunes pour Lerchs-Grossmann)
